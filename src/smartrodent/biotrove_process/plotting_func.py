...
def generate_plots(
    csv_filename, destination_folder, min_threshold=100, max_threshold=1000
):
    """
    ...
    axes1["1,2"].bar(
        category_df["category"], category_df["species_count"], color="skyblue"
    )
    ...
    axes1["2,1"].bar(
        top_40_species["species"], top_40_species["count"], color="lightgreen"
    )
    ...
    total_counts_at_thresholds = [
        df[df["count"] > t]["count"].sum() for t in thresholds
    ]
