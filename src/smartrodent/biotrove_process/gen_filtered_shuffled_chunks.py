...
class GenShuffledChunks:
    ...
    def __init__(self, **kwargs):
        ...
        self.cap_filtered_dir_train = kwargs.get(
            "cap_filtered_dir_train", "cap_filtered_train"
        )
    ...
    def process_files(self):
        ...
        rare_case = set(
            final_counts[final_counts["count"] < self.rare_threshold]["species"]
        )
        frequent_case = set(
            final_counts[final_counts["count"] > self.cap_threshold]["species"]
        )

        for dir_path in [
            self.rare_dir,
            self.cap_filtered_dir_train,
            self.capped_dir,
            self.merged_dir,
        ]:
        ...
        for filename in tqdm(files, desc="Processing files"):
            ...
            frequent_df = capped_filtered_df[
                capped_filtered_df["species"].isin(frequent_case)
            ]
            ...
            for case, count in frequent_case_counts.items():
                ...
                if (
                    frequent_counts[case] > self.cap_threshold
                    and case not in capped_cases
                ):
                    capped_cases.append(case)
                    cap_case_df = frequent_df[frequent_df["species"] == case]
                    cap_case_df.to_parquet(
                        os.path.join(self.capped_dir, f"capped_{case}.parquet"),
                        index=False,
                    )

            capped_df = capped_filtered_df[
                ~capped_filtered_df["species"].isin(capped_cases)
            ]
            df_shuffled = capped_df.sample(
            ...
            df_parts = [
                df_shuffled.iloc[i * rows_per_part : (i + 1) * rows_per_part]
                for i in range(num_parts)
            ]

            if len(df_shuffled) % num_parts != 0:
                df_parts[-1] = pd.concat(
                    [df_parts[-1], df_shuffled.iloc[num_parts * rows_per_part :]],
                    ignore_index=True,
                )

            base_filename, _ = os.path.splitext(filename)
            for i, part in enumerate(df_parts):
                cap_filtered_filepath = os.path.join(
                    self.cap_filtered_dir_train, f"{base_filename}_part{i + 1}.parquet"
                )
                part.to_parquet(cap_filtered_filepath, index=False)

        self.merge_shuffled_files()
        elapsed_time = time.time() - start_time
        # printf"Processing completed in {elapsed_time:.2f} seconds.")
    ...
    def merge_shuffled_files(self):
        ...
        start_time = time.time()
        all_files = sorted(
            glob.glob(os.path.join(self.cap_filtered_dir_train, "*.parquet"))
        )
        random.seed(self.random_seed)
        random.shuffle(all_files)

        for i in tqdm(
            range(0, len(all_files), self.files_per_chunk), desc="Merging files"
        ):
            chunk_files = all_files[i : i + self.files_per_chunk]
            merged_df = pd.concat(
                [pd.read_parquet(file) for file in chunk_files], ignore_index=True
            )
            chunk_filename = os.path.join(
                self.merged_dir,
                f"processed_chunks_{i // self.files_per_chunk + 1:04d}.parquet",
            )
            merged_df.to_parquet(chunk_filename, index=False)
            # printf'Saved merged chunk: {chunk_filename}')

        elapsed_time = time.time() - start_time
        # printf"Merging completed in {elapsed_time:.2f} seconds.")
