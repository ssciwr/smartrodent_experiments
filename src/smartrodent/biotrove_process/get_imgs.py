...
class GetImages:
    ...
    async def process_parquet_file(self, session, semaphore, file_path, output_folder):
        ...
        for future in tqdm(asyncio.as_completed(tasks), total=len(tasks)):
            ...
            if error:
                errors += 1
                # printf"Error downloading image {file_name}: {error}")
        # printf"Completed {file_path} with {errors} errors.")
    ...
    async def download_images(self):
        ...
        async with aiohttp.ClientSession() as session:
            for file_path in files_to_process:
                await self.process_parquet_file(
                    session, semaphore, file_path, self.output_folder
                )
