...
class MetadataProcessor:
    def __init__(self, source_folder, destination_folder, categories=None, **kwargs):
        """
    ...
    def source_folder(self, value):
        ...
        if not glob.glob(os.path.join(value, "*.parquet")):
            raise FileNotFoundError(
                f"No parquet files found in the source folder '{value}'."
            )
    ...
    def destination_folder(self, value):
        ...
        """
        if not os.path.exists(value):
            # # printf"Destination folder '{value}' does not exist. Creating it now.")
    ...
    def _filter_and_count(self, df_parquet):
        ...
        category_filter = (
            ...
            | (df_parquet["order"].isin(self.categories))
            | (df_parquet["kingdom"].isin(self.categories))
            | (df_parquet["family"].isin(self.categories))
            | (df_parquet["genus"].isin(self.categories))
            | (df_parquet["species"].isin(self.categories))
            | (df_parquet["common_name"].isin(self.categories))
            | (df_parquet["scientificName"].isin(self.categories))
        )

        # make tuple of (category, category_type)
        category_tuples = []
        for cat_type in [
            "phylum",
            "class",
            ...
            for cat in self.categories:
                if df_parquet[cat_type].eq(cat).any():
                    category_tuples.append((cat, cat_type))

        df_filtered = df_parquet.loc[category_filter].copy()

        df_filtered.loc[:, "category"] = df_filtered.apply(
            lambda row: next(
                (
                    category
                    for category, category_type in category_tuples
    ...
    def process_single_file(self, file):
        ...
        species_count.to_csv(
            f"{self.destination_folder}/{base_name}_sample_counts_per_species.csv",
            index=False,
        )
        # # printf"Processed file: {file}")
    ...
    def process_all_files(self):
        ...
        with ProcessPoolExecutor() as executor:
            futures = [
                executor.submit(self.process_single_file, parquet_file)
    ...
    def _combine_results(self):
        ...
        """
        species_counts_files = glob.glob(
            os.path.join(self.destination_folder, "*_sample_counts_per_species.csv")
        )
        species_counts = pd.concat([pd.read_csv(file) for file in species_counts_files])
        species_group_counts = (
            species_counts.groupby(["category", "species"])["count"].sum().reset_index()
        )
        species_group_counts.to_csv(
            f"{self.destination_folder}/combined_sample_counts_per_species.csv",
            index=False,
        )
