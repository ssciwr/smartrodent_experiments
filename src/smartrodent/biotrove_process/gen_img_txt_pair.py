...
class GenImgTxtPair:
    ...
    def process_subfolder(self, subfolder_path):
        ...
        if not os.path.exists(parquet_file_path):
            # printf"Parquet file {parquet_file_path} does not exist.")
            return
        ...
        imagelist = [
            int(os.path.basename(i).split(".")[0])
            for i in glob.glob(f"{subfolder_path}/*.jpg")
        ]
        ...
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(
                    self.create_files_and_json, subfolder_path, **row.to_dict()
                )
        ...
        # printf"Text files for {subfolder_name} created in {elapsed_time:.2f} seconds.")
    ...
    def create_image_text_pairs(self):
        ...
        """
        subfolders = [
            os.path.join(self.img_folder, subfolder)
            for subfolder in os.listdir(self.img_folder)
            if os.path.isdir(os.path.join(self.img_folder, subfolder))
        ]
        ...
        for subfolder in subfolders:
            ...
            if self.generate_tar:
                ...
                elapsed_time = time.time() - start_time
                # printf"Tar file for {subfolder_name} created: {tar_path} in {elapsed_time:.2f} seconds.")
